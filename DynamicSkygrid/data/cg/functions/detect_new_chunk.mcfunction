...
execute as @s run function cg:move_to_local_chunk_min

# If a new chunk is found clear it out and generate the skygrid
scoreboard players set @s ds_NewChunk 0
execute store success score @s ds_NewChunk at @s run setblock ~ ~ ~ minecraft:purpur_block replace
    ...
    execute if score @s ds_NewChunk matches 1 as @s run function cg:move_to_local_chunk_min
